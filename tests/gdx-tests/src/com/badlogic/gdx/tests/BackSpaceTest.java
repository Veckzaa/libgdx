...
/** Manual visual test for BACKSPACE key behavior in both desktop and HTML5 backends. Press and hold BACKSPACE to see keyDown (via
 * first keyTyped), repeated PRESSED, and keyUp events asserted. */
		...
		Gdx.input.setInputProcessor(new InputAdapter() {
			...
			public boolean keyTyped (char character) {
				...
				if (character == '\b') {
					if (!sawDown) {
						sawDown = true;
						Gdx.app.log("BackSpaceTest", "DOWN");
					}
					sawPressed = true;
					Gdx.app.log("BackSpaceTest", "PRESSED");
					backSpaceTyped = true;
				}
				return backSpaceTyped;
			}

			@Override
			public boolean keyUp (int keycode) {
				boolean backSpaceUp = false;
